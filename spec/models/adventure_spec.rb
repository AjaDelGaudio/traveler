...
describe Adventure do
  ...
  # it { validates_numericality_of(:latitude).is_greater_than_or_equal_to (-180) }
  # it { validates_numericality_of(:latitude).is_less_than_or_equal_to (180) }
  #
  # it {should allow_value(nil).for(:longitude)}
  # it { should have_valid(:longitude).when(116.4073949999, 116.4073949999) }
  # it { should_not have_valid(:longitude).when('dog', '!') }
  ...
  it { validates_numericality_of(:bucket_list_id) }
end
