...
describe Adventure do
  ...
  it { should have_valid(:address).when(
    'Paris, France',
  ...
  it { should validate_numericality_of(:latitude).is_greater_than_or_equal_to (
    -180
  ) }
  it { should validate_numericality_of(:latitude).is_less_than_or_equal_to (
    180
  ) }

  it { should have_valid(:longitude).when(116.4073949999, 116.4073949999) }
  it { should_not have_valid(:longitude).when("cat", ".") }
  it { should validate_numericality_of(:longitude).is_greater_than_or_equal_to (
    -180
  ) }
  it { should validate_numericality_of(:longitude).is_less_than_or_equal_to (
    180
  ) }
end
